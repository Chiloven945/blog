...
<template>
    <nav
            ...
            class="sticky top-[calc(var(--ui-header-height)+1.5rem)] hidden max-h-[calc(100vh-var(--ui-header-height)-3rem)] overflow-y-auto lg:block"
    >
    ...
    <UCollapsible v-else class="lg:hidden">
        ...
        <template #content>
            <ul class="mt-2 space-y-0.5 border-l border-default pl-1">
                <li v-for="link in flatLinks" :key="link.id">
                    <a
                            :href="`#${link.id}`"
                            :style="{paddingLeft: link.depth > 2 ? '1.5rem' : '0.75rem'}"
